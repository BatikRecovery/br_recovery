/*
	...
	Copyright 2018 Zhantech  
	This file is part of Batik Recovery Project
					...
					*Partition_List)
{
  ...
  if (ListType == "mount")
  ...
  else if (ListType == "wipe")
    {
	  ...
	  gui_parse_text("{@br_wipe_substratum_overlays}");
...
TWPartitionManager::Run_OTA_Survival_Backup (bool adbbackup)
{
  ...
  TWPartition *br = Get_Default_Storage_Partition ();
  if (br)
    DataManager::SetValue ("tw_storage_path", br->Storage_Path);
  else
  ...
  DataManager::GetValue (BR_SURVIVAL_FOLDER_VAR, part_settings.Backup_Folder);
  DataManager::GetValue (BR_SURVIVAL_BACKUP_NAME, Backup_Name);

  part_settings.Backup_Folder =
    part_settings.Backup_Folder + "/" + Backup_Name;
  TWPartition *sys_image =
    PartitionManager.Find_Partition_By_Path ("/system_image");
  if (DataManager::GetIntValue (BR_DO_SYSTEM_ON_OTA) != 0
      && sys_image != NULL)
    Backup_List += "/system_image;/boot;";
  else if (DataManager::GetIntValue (BR_DO_SYSTEM_ON_OTA) != 0)
  ...
  if (DataManager::GetIntValue (BR_DO_SYSTEM_ON_OTA) != 0)
      ...
      if (!disable_free_space_check)
	{
	  if (free_space - (1500 * 1024 * 1024) < total_bytes)
	    {
	      gui_err
		("br_no_space_ota_survival=Not enough free space on internal storage for creating the OTA survival.");
	      return false;
	    }
	}
    }
